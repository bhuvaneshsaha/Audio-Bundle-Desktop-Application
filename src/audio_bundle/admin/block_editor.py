...
class BlockEditor(QWidget):
    ...
    def __init__(self, parent=None) -> None:
        ...
        self._password_hint = QLabel(
            "Course unlock is Custom password. This value is kept for this session only and is not saved in the project file."
        )
    ...
    def _set_enabled(self, enabled: bool) -> None:
        ...
        if enabled:
            ...
            self._password_hint.setVisible(False)

    def sync_auth_ui(self) -> None:
        password = (
            self._workspace is not None
            and self._block_id is not None
            and self._workspace.project.block_auth_method is BlockAuthMethod.PASSWORD
        )

...
* [Course structure (folders and blocks)](docs/COURSE_STRUCTURE.md)
...
2. **Add folder** always creates the next top-level day (**Day 1, Day 2, …**), even if Day 1 is selected. Rename freely. Add **blocks** inside the selected day. Folders are organization only; sequence is per day. Then **Add files** to import MP3/WAV/M4A/AAC or PDF. Drag the ☰ file rows to set playback/display order.
...
3. Unlock blocks with the **course-wide** method the Admin chose (Windows again, custom password, or none). Days can be started in any order; block sequence applies only inside the same day. Unopened blocks show **🔒** (including the first block in a day); **🔓** means currently open; **✓** means opened earlier. Press **F1** for keyboard shortcuts. If single-active is enabled, opening another block locks the previous one.
4. Select a file to decrypt and view it. Closing the bundle deletes the private temporary decrypt folder.

A sample bundle is at `samples/Sample_Course.audiobundle`. Sample passwords are in `samples/admin_project/README.md`; Generate Bundle also writes `*-passwords.txt` next to a new bundle.
